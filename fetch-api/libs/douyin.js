// MediaCrawler douyin.js — A-Bogus & msToken signature generation
...
function pe(e, r, t, n) {
    return 0 <= e && e < 16 ? (r ^ t ^ n) >>> 0 : 16 <= e && e < 64 ? (r & t | r & n | t & n) >>> 0 : void 0;
}

function he(e, r, t, n) {
    return 0 <= e && e < 16 ? (r ^ t ^ n) >>> 0 : 16 <= e && e < 64 ? (r & t | ~r & n) >>> 0 : void 0;
}

function reset() {
    this.reg[0] = 1937774191; this.reg[1] = 1226093241; this.reg[2] = 388252375; this.reg[3] = 3666478592;
    this.reg[4] = 2842636476; this.reg[5] = 372324522; this.reg[6] = 3817729613; this.reg[7] = 2969243214;
    this["chunk"] = []; this["size"] = 0;
}

function write(e) {
    var a = "string" == typeof e ? function(e) {
        n = encodeURIComponent(e)['replace'](/%([0-9A-F]{2})/g, (function(e, r) { return String['fromCharCode']("0x" + r); }));
        var a = new Array(n['length']);
        return Array['prototype']['forEach']['call'](n, (function(e, r) { a[r] = e.charCodeAt(0); })), a;
    }(e) : e;
    this.size += a.length;
    var f = 64 - this['chunk']['length'];
    if (a['length'] < f) this['chunk'] = this['chunk'].concat(a);
    else for (this['chunk'] = this['chunk'].concat(a.slice(0, f)); this['chunk'].length >= 64;)
...
function sum(e, t) {
    ...
    var i = null;
    if (t == 'hex') { i = ""; for (f = 0; f < 8; f++) i += se(this['reg'][f]['toString'](16), 8, "0"); }
    else for (i = new Array(32), f = 0; f < 8; f++) {
        var c = this.reg[f]; i[4 * f + 3] = (255 & c) >>> 0; c >>>= 8;
        i[4 * f + 2] = (255 & c) >>> 0; c >>>= 8; i[4 * f + 1] = (255 & c) >>> 0; c >>>= 8; i[4 * f] = (255 & c) >>> 0;
    }
    return this['reset'](), i;
}
...
function _compress(t) {
    ...
    for (var f = function(e) {
        ...
        var o = le(i[0], 12) + i[4] + le(de(c), c), s = ((o = le(o = (4294967295 & o) >>> 0, 7)) ^ le(i[0], 12)) >>> 0;
        var u = pe(c, i[0], i[1], i[2]); u = (4294967295 & (u = u + i[3] + s + f[c + 68])) >>> 0;
        var b = he(c, i[4], i[5], i[6]); b = (4294967295 & (b = b + i[7] + o + f[c])) >>> 0;
        i[3] = i[2]; i[2] = le(i[1], 9); i[1] = i[0]; i[0] = u; i[7] = i[6]; i[6] = le(i[5], 19); i[5] = i[4]; i[4] = (b ^ le(b, 9) ^ le(b, 17)) >>> 0;
    }
...
function _fill() {
    ...
    for (var i = 0; i < 4; i++) { var c = Math['floor'](a / 4294967296); this['chunk'].push(c >>> 8 * (3 - i) & 255); }
    for (i = 0; i < 4; i++) this['chunk']['push'](a >>> 8 * (3 - i) & 255);
}

function SM3() { this.reg = []; this.chunk = []; this.size = 0; this.reset(); }
SM3.prototype.reset = reset; SM3.prototype.write = write; SM3.prototype.sum = sum;
SM3.prototype._compress = _compress; SM3.prototype._fill = _fill;
...
function generate_rc4_bb_str(url_search_params, user_agent, window_env_str, suffix, Arguments) {
    var sm3 = new SM3();
    var start_time = Date.now();
    var url_search_params_list = sm3.sum(sm3.sum(url_search_params + suffix));
    var cus = sm3.sum(sm3.sum(suffix));
    var ua = sm3.sum(result_encrypt(rc4_encrypt(user_agent, String.fromCharCode.apply(null, [0.00390625, 1, Arguments[2]])), "s3"));
    var end_time = Date.now();
    var b = { 8: 3, 10: end_time, 15: { "aid": 6383, "pageId": 6241, "boe": false, "ddrt": 7, "paths": { "include": [{},{},{},{},{},{},{}], "exclude": [] }, "track": { "mode": 0, "delay": 300, "paths": [] }, "dump": true, "rpU": "" }, 16: start_time, 18: 44, 19: [1, 0, 1, 5] };
    ...
    if (!Arguments) Arguments = [0, 1, 14];
    b[26] = (Arguments[0] >> 24) & 255; b[27] = (Arguments[0] >> 16) & 255; b[28] = (Arguments[0] >> 8) & 255; b[29] = Arguments[0] & 255;
    b[30] = (Arguments[1] / 256) & 255; b[31] = (Arguments[1] % 256) & 255; b[32] = (Arguments[1] >> 24) & 255; b[33] = (Arguments[1] >> 16) & 255;
    b[34] = (Arguments[2] >> 24) & 255; b[35] = (Arguments[2] >> 16) & 255; b[36] = (Arguments[2] >> 8) & 255; b[37] = Arguments[2] & 255;
    ...
    var window_env_list = [];
    for (var index = 0; index < window_env_str.length; index++) { window_env_list.push(window_env_str.charCodeAt(index)); }
    b[64] = window_env_list.length; b[65] = b[64] & 255; b[66] = (b[64] >> 8) & 255;
    b[69] = [].length; b[70] = b[69] & 255; b[71] = (b[69] >> 8) & 255;
...
function sign(url_search_params, user_agent, arguments) {
    var result_str = generate_random_str() + generate_rc4_bb_str(
...
function sign_datail(params, userAgent) { return sign(params, userAgent, [0, 1, 14]); }
function sign_reply(params, userAgent) { return sign(params, userAgent, [0, 1, 8]); }
